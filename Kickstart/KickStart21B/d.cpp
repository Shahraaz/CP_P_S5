...
void solveCase()
{
    ...
    // db(events);
    int timer = 0;
    vector<int> start(n), end(n), lvl(n);
    function<void(int, int)> dfs = [&](int node, int par) -> void {
        start[node] = timer++;
        lvl[node] = node == par ? 0 : lvl[par] + 1;
        for (auto &x : adj[node])
            if (x.first != par)
                dfs(x.first, node);
        end[node] = timer - 1;
        db(node, start[node], end[node]);
    };
    dfs(0, 0);
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        queries[i].first--;
        events.pb({queries[i].second, 1, i});
    }
    sort(all(events));
    // reverse(all(events));
    // db(events);
    ...
    for (auto &e : events)
    {
        ...
        if (e[1] == 0)
        {
            ...
            if (lvl[f] > lvl[s])
                swap(f, s);
            db(f, s, idx, edges[idx]);
            stree.update(start[s], end[s], Node(edges[idx][3]));
        }
        else
        {
            int idx = e[2];
            db(queries[idx]);
            int node = queries[idx].first;
            res[idx] = stree.query(start[node], start[node]).val;
            // cout << stree.query(start[node], start[node]).val << ' ';
        }
    }
    for (auto &x : res)
    {
