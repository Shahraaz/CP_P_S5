...
string encuentrap2 (string s, int &pos){
  ...
  int n = s.size();
  for (int i = 0; i < n; i++){
   for (int j = 1; j < n; j++) if (s[j] == s[0]) {pos = j; return s;}
    s = rota(s, 1); 
  }
  pos = -1;
  return s;
...
string encuentratoros (string s, int &pos1, int &pos2, int &pos3){
  ...
  for (int k = 0; k < n; k++){
   int i, j;
   for (i = 1; i < n; i++) if (igual(s[0], s[i])) break;   
   if (s[0] != s[i]){
     int a['z' - 'a' + 1]; memset (a, 0, sizeof(a));
     for (j = 1; j < i; j++) if (s[j] >= 'a') a[tolower(s[j])-'a']++; else a[tolower(s[j])-'a']--;
     for (j = i + 1; j < n; j++) if (s[j] <= 'Z') a[tolower(s[j])-'a']++; else a[tolower(s[j])-'a']--;       
     char letra = 0;
     for (j = 0; j < 'z' - 'a' + 1; j++) if (a[j] == 2 || a[j] == -2) {letra = (char)('a' + j); break;}     
     if (letra != 0){
       for (j = 1; j < i; j++) if (igual(s[j], letra)) pos1 = j;
       for (j = i + 1; j < n; j++) if (igual(s[j], letra)) pos3 = j;
       pos2 = i;
       return s;
      }
    }           
