...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            q.push_back({{i * j, rnd.next(1000000000)},
                         {i,     j}});
        }
    }
    ...
    int j = 0;
    for (int i = 0;; i++) {
        while (j < q.size() && (q[j].first.first <= q[i].first.first || j - i + 1 < p)) j++;
        if (j == q.size()) break;
        double d = 1.0 * q[j].first.first / q[i].first.first;
        if (q[j].first.first != q[j - 1].first.first)
        if (d < mind) {
            mind = d;
            ii = i;
            jj = j;
...
//    }
    ...
    a[rnd.next(w[0].first - 1)]
    [rnd.next(w[p - 1].second - 1)] = 'A';
    j = 1;
    for (int i = 1; i < p; i++) {
        if (w[i - 1].first < n && w[i].second < m)
            a[w[i - 1].first][w[i].second] = chars[j++];
    }
    assert(j <= k);
