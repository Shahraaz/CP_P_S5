...
/**
* This submission is nearly identical to AC but forgets to set current_tree_size
* current tree to at least 1 if a node is chosen on global level (in try_choose method)
**/
...
void try_choose(node *nodes, ll current, ll *avail) {
  ...
  if(nodes[current].chosen) {
    if(nodes[current].size_needed_right > 0) {
      ...
      // Forget to update the current tree size to at least 1
