...
/**
* This submission always overwrites the current size of the tree
* if we come from the left side.
* Breaks horribly if we overwrite the current size with a smaller one
* but later check if the left tree can support our right tree
**/
...
void generate_depth(node* nodes, ll current, ll depth_to_go) {
  ...
  if(nodes[nodes[current].left].original_tree_size >= depth_to_go - 1) {
    ...
    if(depth_to_go > 2)
      nodes[current].size_needed_right = depth_to_go - 2;
  } else {
    nodes[current].size_needed_right = depth_to_go - 1;
    if(depth_to_go > 2)
  ...
  if(prev < current) {
    ...
    if(check_poss(nodes, nodes[current].parent, current, size + 1, &tmp_avail)) {
      *avail = tmp_avail;
      ...
      nodes[current].current_tree_size = size;    // <---------- ERROR HERE
      return true;
    }
