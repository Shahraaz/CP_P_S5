	...
	FOR(i, 0, k) {
		...
		FOR(j, 1, n) {
			...
			if (a < candidates.size()) {
				...
				if (idx > 0) best = max(best, streak + 1 - min(1., (diffs[j] - diffs[idx-1]) / (diffs[idx] - diffs[idx-1])));
				if (j < n - 1) best = max(best, streak + min(1., (diffs[j] - diffs[idx]) / (diffs[j] - diffs[j+1])));
			}
