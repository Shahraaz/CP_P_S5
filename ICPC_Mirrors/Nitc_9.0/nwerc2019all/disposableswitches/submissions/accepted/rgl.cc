...
int main(){
  ...
  for (int d=n; d>=1; --d) if (~dist[d][n-1]) {
    Line const me={d,(int64_t)dist[d][n-1]};
  ...
  while (not todo.empty()){
    uint64_t d=todo.front().first; int x=todo.front().second; todo.pop();
    answer[x]=true;
    for (auto e: edge[x])
      if (d and not visited[d-1][e.first] and dist[d-1][e.first]!=-1 and dist[d-1][e.first]+e.second==dist[d][x])
