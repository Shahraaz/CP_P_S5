...
while True:
    ...
    e = random.randint(0, len(graph)-1)
    a,b,t,w = graph[e]
    graph[e] = graph[-1]
    graph.pop()
    ...
    for (p,q,t2,frac) in pieces[t]:
        ...
        graph.append((p,q,t2,curw))
        added += 1
    at += cur_vertices-2

    if at > 2000 or len(graph) > 10**4:
    # if at > 200 or len(graph) > 10**3:
    # if at > 10 or len(graph) > 100:
        graph = graph[:-added]
        graph.append((a,b,t,w))
        break
