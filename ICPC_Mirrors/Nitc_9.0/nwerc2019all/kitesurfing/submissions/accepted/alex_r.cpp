...
int main(){
    ...
    int remove_id = 0;
    set<pair<ll,int> > bestchoice;
    vector<vector<pair<ll,int> > > do_remove(K+1);
    cerr << K << endl;
    FOR(i,0,K){
        for(auto it: do_remove[i])bestchoice.erase(it);
        if(!bestchoice.empty()){
            best[i] = min(best[i], bestchoice.begin()->first);
        }
        if(!forbidden[i])best[i+1] = min(best[i+1], best[i] + positions[i+1]-positions[i]);
        bestchoice.insert(make_pair(best[i] + T, i));
        while(remove_id < K && positions[remove_id] - positions[i] <= D)++remove_id;
        do_remove[remove_id].push_back(make_pair(best[i]+T,i));
    }
