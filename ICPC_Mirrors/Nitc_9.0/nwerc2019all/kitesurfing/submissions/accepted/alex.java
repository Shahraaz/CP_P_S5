...
class alex {
  ...
  public static void main(String[] args) {
    ...
    for(int i = n - 2; i >= 0; i--) {
      ...
      long cur_t = 0;
      while(cur_w < n - 1) {
        // it is always optimal to go for the earliest jump directly to finish
        if(jump_to_finish <= water[cur_w][1]) {
          cur_t += t + move(Math.max(0,jump_to_finish - cur_pos), d, t);
          cur_pos = s;
          cur_w = n - 1;
          break;
        }
        ...
        // if we cannot instantly jump to the next body of water, jump until we can
        if(inst_jump == -1) {
          long extra_jumps = (cur_jumptable.get(0) - cur_pos + d - 1) / d;
          cur_t = cur_t + t * extra_jumps;
          cur_pos = Math.min(cur_pos + extra_jumps * d, water[cur_w][1]);
          inst_jump = jump_next_index(cur_jumptable, cur_pos);
        } 
