...
public:
    ...
    void solveCase()
    {
        ...
        c++;
        Segtree<Node> S(n + 1);
        vector<int> maxPossibleWorkDays(n + 1);
        for (int i = n - 1; i >= 0; i--)
        {
            maxPossibleWorkDays[i] = S.query(i + 1, n).val;
            if (str[i] == 'o')
            {
                if (i + c < n)
                {
                    maxPossibleWorkDays[i] = max((ll)maxPossibleWorkDays[i], 1 + S.query(i + c, n).val);
                }
                maxPossibleWorkDays[i] = max(maxPossibleWorkDays[i], 1);
            }
            db(i, maxPossibleWorkDays[i]);
            S.update(i, Node(maxPossibleWorkDays[i]));
        }
        vector<int> canSkip(n + 1);
        for (size_t i = 0; i < n; i++)
        {
            if (maxPossibleWorkDays[i] > k)
                canSkip[i]++, canSkip[i + 1]--;
            else
            {
                if (str[i] == 'o')
                {
                    if (S.query(i + 1, n).val >= k)
                        canSkip[i]++, canSkip[i + 1]--;
                    if ((S.query(i + c, n).val + 1) == maxPossibleWorkDays[i])
                    {
                        canSkip[i + 1]++, canSkip[i + c - 1]--;
                    }
                }
            }
        }
