...
void Solution::solveCase()
{
    int H, W, M;
    cin >> H >> W >> M;
    // vector<int> rowsum(H + 1), colsum(W + 1);
    Segtree<Node> rowSum(W + 1);
    vector<int> adj[H + 1];
    ...
    for (size_t i = 1; i <= H; i++)
    {
        ...
        int temp = adj[i].size() + rowSum.query(adj[i].front(), adj[i].back()).val;
        maxi = max(maxi, temp);
        temp = adj[i].size() + rowSum.query(0, adj[i].front() - 1).val;
        maxi = max(maxi, temp);
        temp = adj[i].size() + rowSum.query(adj[i].back(), W).val;
        maxi = max(maxi, temp);
